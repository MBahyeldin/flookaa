...
export function useToolbarSchema(props: {
  ...
  const schema = useEditorSelector(
    editor,
    (snapshot: { context: { schema: ToolbarSchema } }) => snapshot.context.schema
  );

  return {
    decorators: schema.decorators.map(
      (decorator: ToolbarDecoratorSchemaType) => props.extendDecorator?.(decorator) ?? decorator
    ),
    annotations: schema.annotations.map(
      (annotation: ToolbarAnnotationSchemaType) => props.extendAnnotation?.(annotation) ?? annotation
    ),
    lists: schema.lists.map((list: ToolbarListSchemaType) => props.extendList?.(list) ?? list),
    blockObjects: schema.blockObjects.map(
      (blockObject: ToolbarBlockObjectSchemaType) => props.extendBlockObject?.(blockObject) ?? blockObject
    ),
    inlineObjects: schema.inlineObjects.map(
      (inlineObject: ToolbarInlineObjectSchemaType) => props.extendInlineObject?.(inlineObject) ?? inlineObject
    ),
    styles: schema.styles.map((style: ToolbarStyleSchemaType) => props.extendStyle?.(style) ?? style),
  };
}
